...
const MoviesPosterandInfo = (props) => {
  const { navigation, data, type } = props
      ...
      >
        <View style={{ flexDirection: "row" }}>
          ...
          <View style={{ margin: 16, justifyContent: "center", marginBottom: 24, flex: 1 }}>
            <Text style={{ fontWeight: "bold", fontSize: 16, marginBottom: 10, color: white }} numberOfLines={2}>
              {data.name}
              {data.title}
            </Text>
            <MovieRating rating={data.vote_average} textColor={white} />
            <Text style={{ fontWeight: "300", fontSize: 12, marginTop: 10, width: "75%", color: white }}>
